...
public:
  ReplayPathPlanner(double lookahead_distance)
      : m_lookahead_distance(lookahead_distance) {}

  vector<vector<double>> get_replayed_path(vector<double> ego_state,
                                           vector<vector<double>> global_route);

  vector<vector<double>>
  get_interpolated_path(vector<vector<double>> replayed_path,
                        double interp_res = 0.01);
...
private:
  ...
  int get_lookahead_index(double x, double y, vector<vector<double>> waypoints,
