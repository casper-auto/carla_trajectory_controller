...
ReplayPathPlanner::get_replayed_path(vector<double> current_pose,
  ...
  m_lookahead_index =
      get_lookahead_index(x, y, global_route, m_lookahead_distance);
  return slice(global_route, m_closest_index, m_lookahead_index);
}

vector<vector<double>>
ReplayPathPlanner::get_interpolated_path(vector<vector<double>> replayed_path,
                                         double interp_res) {
  ...
  for (int i = 0; i < replayed_path.size() - 1; i++) {
    ...
    for (int j = 0; j < num_pts_to_interp; j++) {
      vector<double> next_wp_vector = {
          interp_res * float(j + 1) * wp_uvector[0],
...
int ReplayPathPlanner::get_closest_index(double x, double y,
...
int ReplayPathPlanner::get_lookahead_index(double x, double y,
                                           vector<vector<double>> waypoints,
