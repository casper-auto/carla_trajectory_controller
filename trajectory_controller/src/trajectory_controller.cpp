...
int TrajectoryController::get_goal_waypoint_index(
    double x, double y, vector<vector<double>> waypoints,
...
double TrajectoryController::calculate_steering(
    double t, double x, double y, double yaw, vector<vector<double>> waypoints,
    double v) {
  double steering = 0;
...
void TrajectoryController::update_controls(double lag_throttle,
                                           double lag_brake,
                                           double lag_steering,
  ...
  if (abs(m_vars["t_previous"] - t) < 0.001)
    return;
  ...
  if (!m_vars.count("v_previous"))
    m_vars["v_previous"] = 0.0;
  if (!m_vars.count("t_previous"))
    m_vars["t_previous"] = 0.0;
  if (!m_vars.count("prev_throttle"))
    m_vars["prev_throttle"] = 0.0;
  if (!m_vars.count("v_error_previous"))
    m_vars["v_error_previous"] = 0.0;
  if (!m_vars.count("alpha_previous"))
    m_vars["alpha_previous"] = 0.0;
  if (!m_vars.count("steering_previous"))
    m_vars["steering_previous"] = 0.0;
